...
type Preparer interface {
	// Prepare 在共享启动 Context 内准备资源，但不对外启动服务。
	Prepare(context.Context) error
}

// Starter 启动组件持有的服务或后台能力。只有成功 Start 的组件才会在回滚时 Stop。
type Starter interface {
	// Start 在所有依赖 Prepare 成功后启动当前组件。
...
type Runner interface {
	// Run 持续执行直到 Context 取消或发生真实业务错误。
	Run(context.Context) error
}

// Stopper 停止已经成功启动的组件，按依赖逆序调用，保证消费者先于依赖退出。
type Stopper interface {
	// Stop 在共享关停 Context 内停止已成功启动的能力。
