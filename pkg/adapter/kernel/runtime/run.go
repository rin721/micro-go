...
func (a *Application) Run(ctx context.Context) error {
	...
	for primary == nil {
		...
		case result := <-runnerResults:
			...
			if result.err != nil {
				_ = a.emit(kernelapp.Event{Kind: kernelapp.RunnerFailed, State: kernelapp.Running, Phase: diagnostic.Run, Err: result.err})
				primary = result.err
...
func (a *Application) reload(ctx context.Context) error {
	...
	candidate, err := a.runtime.dependencies.Loader.Load(ctx, a.snapshot.Version+1, a.options.sources, a.plan.Configs)
	if err != nil {
	...
	for _, instance := range a.instances {
		...
		if !ok {
			...
			return kernelapp.ErrRestartRequired
		}
...
func (a *Application) shutdown(cancel context.CancelFunc, started []bool, runnerResults <-chan runnerResult, runnersRemaining int, primary error, final kernelapp.State) error {
	...
	_ = a.setState(kernelapp.Stopping, diagnostic.Stop)
	...
	_ = a.setState(kernelapp.Closing, diagnostic.Close)
	...
	if len(errorsList) > 0 && final == kernelapp.Closed {
		final = kernelapp.Failed
	}
	a.stateValue.Store(uint32(final))
	_ = a.emit(kernelapp.Event{Kind: kernelapp.StateChanged, State: final})
